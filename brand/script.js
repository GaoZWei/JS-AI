...
window.onload = async () => {
    ...
    const surface = tfvis.visor().surface({ name: '输入示例', styles: { height: 250 } });//抽屉组件
    inputs.forEach(img => {
        surface.drawArea.appendChild(img);
    });

    const mobilenet = await tf.loadLayersModel(MOBILENET_MODEL_PATH);//mobilenet预训练模型
    mobilenet.summary();
    ...
    model.add(tf.layers.flatten({//摊平为一维向量
        inputShape: layer.outputShape.slice(1)//从第二个到后面的
    }));
    ...
    const { xs, ys } = tf.tidy(() => {//先把数据喂给truncatedMobilenet
